...
// IPNetSet maps string to netip.AddrNet.
type IPNetSet map[string]netip.Prefix
...
func ParseIPNets(specs ...string) (IPNetSet, error) {
	...
	for _, spec := range specs {
		...
		ipnet, err := netip.ParsePrefix(spec)
		if err != nil {
			return nil, err
		}
...
func (s IPNetSet) Insert(items ...netip.Prefix) {
...
func (s IPNetSet) Delete(items ...netip.Prefix) {
	for _, item := range items {
...
func (s IPNetSet) Has(item netip.Prefix) bool {
	_, contained := s[item.String()]
	return contained
}

// HasAll returns true if and only if all items are contained in the set.
func (s IPNetSet) HasAll(items ...netip.Prefix) bool {
	for _, item := range items {
		if !s.Has(item) {
			return false
		}
...
// IPSet maps string to netip.Addr
type IPSet map[string]netip.Addr

// ParseIPSet parses string slice to IPSet
func ParseIPSet(items ...string) (IPSet, error) {
	ipset := make(IPSet)
	for _, item := range items {
		ip, _ := netip.ParseAddr(strings.TrimSpace(item))
		if !ip.IsValid() {
...
// Insert adds items to the set.
func (s IPSet) Insert(items ...netip.Addr) {
	for _, item := range items {
		s[item.String()] = item
	}
}
...
// Delete removes all items from the set.
func (s IPSet) Delete(items ...netip.Addr) {
	for _, item := range items {
		delete(s, item.String())
	}
}
...
// Has returns true if and only if item is contained in the set.
func (s IPSet) Has(item netip.Addr) bool {
	_, contained := s[item.String()]
	return contained
}

// HasAll returns true if and only if all items are contained in the set.
func (s IPSet) HasAll(items ...netip.Addr) bool {
	for _, item := range items {
		if !s.Has(item) {
			return false
		}

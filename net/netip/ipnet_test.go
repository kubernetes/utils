...
func parseIPNet(s string) netip.Prefix {
...
func TestIPSet(t *testing.T) {
	...
	c := netip.MustParseAddr("3.0.0.0")
	d := netip.MustParseAddr("4.0.0.0")
...
func TestParseIPSet(t *testing.T) {
	...
	b := netip.MustParseAddr("2.0.0.0")
	c := netip.MustParseAddr("3.0.0.0")
	e := netip.MustParseAddr("4.0.0.0")

	if !s.Has(a) || !s.Has(b) || !s.Has(c) || !s.Has(e) {
		t.Errorf("Unexpected contents: %#v", s)
	}
}
...
func TestIPSetDifference(t *testing.T) {
	...
	if !c.Has(netip.MustParseAddr("3.0.0.0")) {
		t.Errorf("Unexpected contents: %#v", c)
	}
	if len(d) != 2 {
		t.Errorf("Expected len=2: %d", len(d))
	}
	if !d.Has(netip.MustParseAddr("4.0.0.0")) || !d.Has(netip.MustParseAddr("5.0.0.0")) {
		t.Errorf("Unexpected contents: %#v", d)
	}
}
